...
(defn restar [estado]
  (let [valor1 (obtener-ultimo estado)
    ...
    (apilar estado-sin-valor2 (- valor2 valor1))))  ; Restar: value1 - value2
...
(defn intercambiar [estado]
  (let [pila (:pila estado)]
    (if (< (count pila) 2)
      estado ;; Si hay menos de dos elementos, devolvemos el estado sin cambios.
      (let [valor2 (obtener-ultimo estado)
            estado-sin-valor2 (desapilar estado)   ;; Desapila una vez
            valor1 (obtener-ultimo estado-sin-valor2)
            estado-sin-valor1 (desapilar estado-sin-valor2)] ;; Desapila otra vez
        (apilar (apilar estado-sin-valor1 valor2) valor1))))) ;; Apila los valores intercambiados
  ...
  (let [pila (:pila estado)]
    (if (< (count pila) 2)
      ...
      (let [y (obtener-ultimo estado)
            ...
            valor-en-pos (get-in matriz [x y])]
        (apilar estado-sin-x valor-en-pos)))))
  ...
  (let [pila (:pila estado)]
    (if (< (count pila) 3)
      ...
      (let [y (obtener-ultimo estado)
            ...
            matriz (:matriz estado)
            matriz-actualizada (assoc-in matriz [x y] value)]
...
(defn -main []
  ...
  (let [contenido (leer-archivo (buscar-archivo "ejemplo.txt")) ;; Leer el archivo
    ...
    ;      (prn fila))
    (ejecutar-programa estado-con-matriz)))
